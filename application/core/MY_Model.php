<?php class MY_Model extends CI_Model
{
    ...
    public function check_by($where, $tbl_name)
    {
        ...
        $this->db->where($where);
        $query_result = $this->db->get();
        return $query_result ? $query_result->row() : null;
    }
    ...
    public function get_result($where, $tbl_name)
    {
        ...
        $this->db->where($where);
        $query_result = $this->db->get();
        return $query_result ? $query_result->result() : array();
    }
    ...
    {
        if (!$this->db->table_exists($table) || !$this->db->field_exists($table_field, $table)) {
            return null;
        }
    ...
    function get_table_field($tableName, $where = array(), $field = null)
    {
        if (empty($field)) {
            return false;
        }
        $row = $this->db->select($field)->where($where)->get($tableName)->row();
        return ($row && isset($row->$field)) ? $row->$field : false;
    }
