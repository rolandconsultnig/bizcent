...
{
    $CI = &get_instance();
    if (!$CI->db->table_exists('tbl_custom_field') || !$CI->db->table_exists('tbl_form')) {
        return;
    }
...
{
    // Re-install is already blocked by install/index.php once install.php is removed.
    // Do not halt the app just because the install directory is still on disk.
    return;
}
...
{
    $CI = &get_instance();
    if (!$CI->db->table_exists($table) || !$CI->db->field_exists($table_field, $table)) {
        return !empty($result) ? array() : null;
    }
...
function join_data($table, $select = '*', $where = null, $join = null, $row = null)
{
    ...
    $CI = &get_instance();
    $empty = (!empty($row) && ($row === 'array' || $row === 'object')) ? array() : null;
    if (!$CI->db->table_exists($table)) {
        return $empty;
    }
    if ($select == '*') {
    ...
    if (!empty($join)) {
        foreach ($join as $tbl => $wh) {
            $join_table = preg_replace('/\s.*/', '', $tbl);
            if (!$CI->db->table_exists($join_table)) {
                continue;
            }
