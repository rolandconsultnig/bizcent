...
class Common_model extends MY_Model
{
    ...
    public function sales_pdf($data, $pdf)
    {
        ...
        if ($pdf === 'attach') {
            $result = pdf_create($viewfile, slug_it($data['title'] . '_pdf_' . $data['sales_info']->reference_no), 1, null, true);
            return $result;
        } else {
            pdf_create($viewfile, slug_it($data['title'] . '_pdf_' . $data['sales_info']->reference_no));
        }
    }
