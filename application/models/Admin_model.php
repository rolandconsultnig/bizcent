<?php class Admin_Model extends MY_Model
{
    ...
    { // this function is to create get monthy recap report
        ...
        $get_goal_report = array();
        if ($this->db->table_exists('tbl_goal_type') && $this->db->count_all('tbl_goal_type') > 0) {
            $get_goal_report = $this->admin_model->get_goal_report_by_month_2($start_date, $end_date);
        }
...
GROUP BY tbl_goal_type.goal_type_id
  ")->result();
    ...
    {
        if (!$this->db->table_exists('tbl_payments') || !$this->db->field_exists('month_paid', 'tbl_payments')) {
            return 0;
        }
    ...
    {
        $this->db->limit(1);
        $row = $this->db->get('tbl_migrations')->row();
        return $row && isset($row->version) ? $row->version : 0;
    }
