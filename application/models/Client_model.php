...
class Client_Model extends MY_Model
{
    ...
    public function get_client_contacts($client_id)
    {
        ...
        if (!empty($result) && $primary_contact == 0 || empty($primary_contact)) {
            $client_data['primary_contact'] = !empty($result[0]->user_id) ? $result[0]->user_id : '';
    ...
    public function getClientData($tbl, $id, $count = null)
    {
        // get client tickets from tbl_tickets,join tbl_account_details where tbl_tickets.reporter = tbl_account_details.company
        $this->db->select($tbl . '.*,tbl_account_details.fullname,tbl_account_details.company');
        $this->db->from($tbl);
        $this->db->join('tbl_account_details', $tbl . '.reporter = tbl_account_details.user_id', 'left');
        $this->db->where('tbl_account_details.company', $id);
        $query = $this->db->get();
        // count all client tickets
        if (!empty($count)) {
