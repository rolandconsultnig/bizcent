    <?php
    $project_timers = array();
    $task_timers = array();
    if ($this->db->field_exists('timer_status', 'tbl_project')) {
        ...
        if (!is_array($project_timers)) {
            $project_timers = array();
        }
    }

    if ($this->db->field_exists('timer_status', 'tbl_task')) {
