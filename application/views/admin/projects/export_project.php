...
$comment_details = total_rows('tbl_task_comment', array('module', 'projects', 'module_field_id', $project_details->project_id));
$where = array('project_id' => $project_details->project_id);
$all_milestones_info = total_rows('tbl_milestones', $where);
$total_tasks = total_rows('tbl_task', $where);
...
$all_estimates_info = total_rows('tbl_estimates', array('project_id' => $project_details->project_id));
    ...
    <tr>
        ...
        <td style="width: 30%;float: right">
            ...
            <div class="name"><strong><?= lang('total') . ' ' . lang('comments') ?>
                    :</strong> <?= (!empty($comment_details) ? $comment_details : 0) ?></div>
            ...
            <div class="name"><strong><?= lang('total') . ' ' . lang('tasks') ?>
                    :</strong> <?= (!empty($all_task_info) ? $total_tasks : 0) ?></div>
            ...
            <div class="name"><strong><?= lang('total') . ' ' . lang('estimates') ?>
                    :</strong> <?= (!empty($all_estimates_info) ? ($all_estimates_info) : 0) ?></div>
        </td>
...
<table class="items" width="100%">
    ...
    foreach ($permission_users as $v_users) {
        ...
        $cwhere = array('user_id' => $v_users->user_id, 'module' => 'projects', 'module_field_id' => $project_details->project_id);
        $total_u_comments = total_rows('tbl_task_comment', $cwhere);
        $total_u_attachment = total_rows('tbl_attachments', $cwhere);
