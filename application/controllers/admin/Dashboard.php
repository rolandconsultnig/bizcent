...
class Dashboard extends Admin_Controller
{
    ...
    public function different_completed_total()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->db->table_exists('tbl_estimate_items')) {
                $est_row = $this->db->query("SELECT  tbl_estimates.estimates_id, sum(tbl_estimate_items.total_cost) as cost
        FROM tbl_estimates
        ...
        WHERE tbl_estimates.status NOT IN ('draft', 'cancelled')")->row();
                $total_estimate_amount = ($est_row && !empty($est_row->cost)) ? $est_row->cost : 0;
            }
    ...
    public function recent_activities()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $activity_date_col = $this->db->field_exists('activity_date', 'tbl_activities') ? 'tbl_activities.activity_date' : 'tbl_activities.time';
            $data['activities'] = $this->db->query("SELECT $activity_date_col AS activity_date, tbl_activities.activity,tbl_activities.value1, tbl_activities.value2, tbl_account_details.avatar, tbl_account_details.fullname
        FROM tbl_activities 
        LEFT  JOIN tbl_account_details ON tbl_activities.user = tbl_account_details.user_id
        $whereSql
        ORDER BY $activity_date_col DESC LIMIT 10")->result();
