...
class Invoice extends Admin_Controller
{
    ...
    public function invo_data()
    {
        ...
        $zero = display_money(0, default_currency());
        $empty = array(
            'client_outstanding' => $zero,
            'total_invo_amount' => $zero,
            'total_invoice_amount' => $zero,
            'today_invo_amount' => $zero,
            'paid_invo_amount' => $zero,
            'total_receipts' => $zero,
            'payment_today' => $zero,
            'past_overdue' => $zero,
            'invoice_overdue_no' => 0,
        );
        if (!$this->db->table_exists('tbl_items') || !$this->db->field_exists('inv_deleted', 'tbl_invoices')) {
